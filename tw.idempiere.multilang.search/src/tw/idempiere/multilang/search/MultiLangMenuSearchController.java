...
/**
 * Multi-language search controller for AD_Menu records.
 * Extends MenuSearchController to add cross-language menu search.
 * Searches menu items across all translated languages in AD_Menu_Trl.
 *
 * COPIED FROM MenuSearchController.java (iDempiere 14.0) with modifications marked below.
 * === MODIFICATION markers indicate changed sections ===
 * @author hengsin
 */
public class MultiLangMenuSearchController extends MenuSearchController {
	...
	// === MODIFICATION START: multi-language search fields ===
	private static final Logger log = Logger.getLogger(MultiLangMenuSearchController.class.getName());
	/** AD_Menu_ID → list of alternative language names */
	 ...
	 */
	public MultiLangMenuSearchController(Tree tree) {
		super(tree);
		this.tree = tree;
	}
	...
	public void refreshModel() {
		...
		sortMenuItemModel();		
		moveRecentItems();
		// === MODIFICATION START: load alternative language labels ===
		loadAlternativeLabels();
		// === MODIFICATION END ===
	}

	// === MODIFICATION START: multi-language label loading ===
	/**
	 * Load menu names from all languages (except current session language)
	...
	private void loadAlternativeLabels() {
		...
		// 1. Load from AD_Menu_Trl (translated names in other languages)
		...
		// 2. Load base table names (when session language is not the base language)
		...
		int totalLabels = altLabelsMap.values().stream().mapToInt(List::size).sum();
		log.info("Multi-Language Search: loaded " + totalLabels + " alternative labels for " + altLabelsMap.size() + " menu items");
	}

	/**
	 * Extract AD_Menu_ID from a MenuItem's data field.
	 * Handles both DefaultTreeNode (model-based tree) and Treeitem (component-based tree).
	...
	// === MODIFICATION START: multi-language comparator ===
	/**
	 * Comparator that filters menu items by matching against the current language
	 * label first, then against all alternative language labels from AD_Menu_Trl.
	 */
	private class MenuListComparator implements Comparator<MenuItem> {
		...
		public int compare(MenuItem o1, MenuItem o2) {
			...
			// 1. Match against current language label (original behavior)
			...
			// 2. Match against alternative language labels
	...
	private class MenuItemRenderer implements ListitemRenderer<MenuItem>, ListitemRendererExt {
		...
		public void render(Listitem item, MenuItem data, int index)
			...
			item.setValue(data);
			item.addEventListener(Events.ON_CLICK, MultiLangMenuSearchController.this);
			...
			if (node != null) {
				...
				starBtn.addEventListener(Events.ON_CLICK, MultiLangMenuSearchController.this);
